...
public static class CliHost
{
    public static int Run(string[] args)
    {
        ...
        try
        {
            ...
            // Handle direct action commands (e.g., mouse_move, window_activate, volume_set, screenshot)
            result = command switch
            {
                "mouse" => HandleMouse(subArgs),
                "keyboard" => HandleKeyboard(subArgs),
                "screen" => HandleScreen(subArgs),
                "window" => HandleWindow(subArgs),
                "system" => HandleSystem(subArgs),
    ...
    private static Response HandleMouse(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("Mouse subcommand required");
        ...
        {
            "move" => MouseModule.Move(new MouseMoveRequest
            {
                X = GetIntOption(options, "x", 0),
                Y = GetIntOption(options, "y", 0),
                Relative = GetBoolOption(options, "relative", false),
            ...
            "click" => MouseModule.Click(new MouseClickRequest
            {
                Button = ParseMouseButton(GetStringOption(options, "button", "left")),
                Double = GetBoolOption(options, "double", false)
            }),
            "down" => MouseModule.Down(ParseMouseButton(GetStringOption(options, "button", "left"))),
            "up" => MouseModule.Up(ParseMouseButton(GetStringOption(options, "button", "left"))),
            "scroll" => MouseModule.Scroll(new MouseScrollRequest
            {
                Amount = GetIntOption(options, "amount", 0),
                Horizontal = GetBoolOption(options, "horizontal", false)
            }),
            "position" => MouseModule.GetPosition(),
            _ => new ErrorResponse($"Unknown mouse command: {subCmd}")
        };
    }

    private static Response HandleKeyboard(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("Keyboard subcommand required");
        ...
        return subCmd switch
        {
            "press" => KeyboardModule.Press(new KeyPressRequest
            {
                Key = GetStringOption(options, "key", "")!
            }),
            "type" => KeyboardModule.TypeText(new TypeTextRequest
            {
                Text = GetStringOption(options, "text", "")!,
                Interval = GetIntOption(options, "interval", 0),
                HumanLike = GetBoolOption(options, "human", false)
            }),
            _ => new ErrorResponse($"Unknown keyboard command: {subCmd}")
        };
    }

    private static Response HandleScreen(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("Screen subcommand required");
        ...
        return subCmd switch
        {
            ...
            "pixel" => ScreenModule.GetPixelColor(new PixelColorRequest
            {
                X = GetIntOption(options, "x", 0),
                Y = GetIntOption(options, "y", 0)
            }),
            "list" => ScreenModule.ListScreens(),
            _ => new ErrorResponse($"Unknown screen command: {subCmd}")
        };
    }

    private static Response HandleWindow(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("Window subcommand required");
        ...
        return subCmd switch
        {
            "list" => WindowModule.List(new WindowListRequest
            {
                VisibleOnly = GetBoolOption(options, "visible-only", true),
                TitleFilter = GetStringOption(options, "filter", null)
            }),
            "find" => WindowModule.Find(new WindowFindRequest
            {
                Title = GetStringOption(options, "title", null),
                ClassName = GetStringOption(options, "class", null),
                ProcessId = GetNullableUintOption(options, "pid")
            }),
            "activate" => ActivateWindowByTitleOrHandle(options),
            "foreground" => WindowModule.GetForeground(),
            _ => new ErrorResponse($"Unknown window command: {subCmd}")
        };
    }

    private static Response HandleSystem(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("System subcommand required");
        ...
        return subCmd switch
        {
            ...
            "disk" => SystemModule.ListDisks(),
            "processes" => SystemModule.ListProcesses(new ProcessListRequest
            {
                NameFilter = GetStringOption(options, "filter", null)
            }),
            "lock" => SystemModule.LockScreen(),
            _ => new ErrorResponse($"Unknown system command: {subCmd}")
        };
    }

    private static Response HandleClipboard(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("Clipboard subcommand required");
        ...
        return subCmd switch
        {
            "get" => ClipboardModule.GetText(new ClipboardGetTextRequest()),
            "set" => ClipboardModule.SetText(new ClipboardSetTextRequest
            {
                Text = GetStringOption(options, "text", "")!
            }),
            "clear" => ClipboardModule.Clear(new ClipboardClearRequest()),
            "files" => ClipboardModule.GetFiles(new ClipboardGetFilesRequest()),
            _ => new ErrorResponse($"Unknown clipboard command: {subCmd}")
        };
    }

    private static Response HandleAudio(string[] args)
    {
        if (args.Length == 0) return new ErrorResponse("Audio subcommand required");
        ...
        return subCmd switch
        {
            "volume" => options.ContainsKey("level") 
                ? AudioModule.SetVolume(new VolumeSetRequest { Level = GetIntOption(options, "level", 50) })
                : AudioModule.GetVolume(new VolumeGetRequest()),
            "mute" => AudioModule.SetMute(new VolumeMuteRequest { Mute = GetBoolOption(options, "state", true) }),
            "devices" => AudioModule.ListDevices(new AudioDeviceListRequest()),
            _ => new ErrorResponse($"Unknown audio command: {subCmd}")
        };
    }
    ...
    private static nint GetNintOption(Dictionary<string, string> options, string key, nint defaultValue)
    {
        return options.TryGetValue(key, out var value) && nint.TryParse(value, out var result) ? result : defaultValue;
    }
    ...
    {
        Console.WriteLine("EasyTouch Windows Automation Tool");
        Console.WriteLine();
